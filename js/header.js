...
if (!e.composedPath().includes(ModalSearch) && !e.composedPath().includes(OpenSearchBtn)) {
  ModalSearch.classList.add("-translate-y-[149rem]");
}  
if(!e.composedPath().includes(Sidebar) && !e.composedPath().includes(OpenSidebarBtn)){
  Sidebar.classList.remove("translate-x-0");
}
if(!e.composedPath().includes(CartWrapper) && !e.composedPath().includes(OpenCartBtn)){
  CartWrapper.classList.add("translate-x-[500px]");
}
if(!e.composedPath().includes(OpenLoginForm) && !e.composedPath().includes(FormWrapper)){
